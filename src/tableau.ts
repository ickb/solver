...
export type Variables<VarKey = string, ConKey = string> = readonly (readonly [
  VarKey,
  Coefficients<ConKey>,
])[];

// A tableau with some additional context.
export interface TableauModel<VariableKey = string, ConstraintKey = string> {
...
const convertToIterable = <K, V>(
  seq:
    ...
    | ([K] extends [string] ? Readonly<Partial<Record<K, V>>> : never),
): Iterable<readonly [K, V]> =>
...
export const tableauModel = <VarKey = string, ConKey = string>(
  ...
  const variables: Variables<VarKey, ConKey> = Array.isArray(variablesIter)
    ? variablesIter
  ...
  if (integers != null || binaries != null) {
    ...
    const integerVariables =
      binaryVariables === true ? true : convertToSet(integers);
  ...
  const constraints = new Map<
    ConKey,
    { row: number; lower: number; upper: number }
  >();
  for (const [key, constraint] of constraintsIter) {
    ...
    bounds.lower = Math.max(
      bounds.lower,
      constraint.equal ?? constraint.min ?? -Infinity,
    );
    bounds.upper = Math.min(
      bounds.upper,
      constraint.equal ?? constraint.max ?? Infinity,
    );
